...
r1 = 0;
%%%%%%%%%%%%%%%%%%%%%%%%% taking input %%%%%%%%%%%%%%%%%%%%%%%%%
prompt = {'Enter length calibration factor (mm/pixel)','Enter frame rate in f.p.s.','enter radius calibration factor'};
dlgtitle = 'Input';
dims = [1 50];
definput = {'1','10000','5'};  %(197/7.5)^-1 as provided
...
for cnt = check:l %replace by l to iterate 
    ...
    figure(3)
    imshowpair(gray1,CH1)
    dumy = regionprops(CH1,"Area","Centroid","Perimeter");
    area = [area; dumy.Area];
    peri = [peri; dumy.Perimeter];
    coa = [coa; dumy.Centroid];
    xp = [xp; cord_of_sprayxpos(CH1)];
    xn = [xn; cord_of_sprayxneg(CH1)];
    yp = [yp; cord_of_sprayypos(CH1)];
    yn = [yn; cord_of_sprayyneg(CH1)];
end
